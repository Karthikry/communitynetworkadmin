...
const columns = [
  ...
  { id: 'acceptedBy', label: 'Accepted By', minWidth: 180 },
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Set rows and total row count from API response
    ...
    fetchData();
  }, [page, rowsPerPage]); // Add page and rowsPerPage as dependencies to refetch on page change
            ...
            {rows.length > 0 ? (
              rows.map((row, index) => (
                <TableRow hover role="checkbox" tabIndex={-1} key={index}>
                  ...
                  <TableCell>{row.status || 'Not Available'}</TableCell>
                  <TableCell>{row.insertedDate ? format(new Date(row.insertedDate), 'dd MMM yyyy') : 'Not Available'}</TableCell>
                  
                  <TableCell>{row.acceptedBy?.fullName || 'Not Available'}</TableCell>
                  <TableCell>{row.updatedBy?.fullName || 'Not Available'}</TableCell>
                  <TableCell>{row.requestedBy?.userName || 'Not Available'}</TableCell>
                  <TableCell>{row.requestTypeDto?.requestTypeName || 'Not Available'}</TableCell>
                </TableRow>
              ))
            ) : (
              <TableRow>
...
//          const response = await fetch(
//            `https://executivetracking.cloudjiffy.net/Mahaasabha/request/v1/getAllRequestByPagination/{pageNumber}/{pageSize}?pageNumber=${rowsPerPage}&pageSize=${pageSize}`,
//            {
//              method: 'GET',
//              headers: {
//                'Content-Type': 'application/json',
